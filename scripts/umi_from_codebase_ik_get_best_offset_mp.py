...
r3_ik_solver.max_iter = 100
r4_ik_solver.max_iter = 100
r3_ik_solver.reset()
r4_ik_solver.reset()
...
with zarr.ZipStore(zarr_path) as zip_store:
    ...
    robot0_eef_rot = np.array(data_group['robot0_eef_rot_axis_angle'])
    robot0_gripper_width = np.array(data_group['robot0_gripper_width'])

    # robot1_demo_end_pose = data_group['robot1_demo_end_pose']
    # robot1_demo_start_pose = data_group['robot1_demo_start_pose']
    robot1_eef_pos = np.array(data_group['robot1_eef_pos'])
    robot1_eef_rot = np.array(data_group['robot1_eef_rot_axis_angle'])
    robot1_gripper_width = np.array(data_group['robot1_gripper_width'])

    if RENDER:
        camera1_rgb = np.array(data_group['camera1_rgb'])
        camera0_rgb = np.array(data_group['camera0_rgb'])
...
def process_ziggle(zig_idx):
    ...
    total_errs = []
    for i in tqdm(range(0, num_ep), desc=f"Processing ziggle {zig_idx+1}/{ziggle.shape[0]}"):
        # result_dir = glob.glob(os.path.join(zig_dir, f"ep_{i:02d}_results") + '/*')
        ...
        for t in range(ep_start, ep_end):
            ...
            if RENDER:
                im0 = camera1_rgb[left_t]
                im1 = camera0_rgb[right_t]
            ...
            if t == ep_start:
                ...
                r3_ik_solver.max_iter = 30
                r4_ik_solver.max_iter =30
        ...
        large_qchange_l = np.sum(large_qchanges[:, 0])/len(large_qchanges)
        large_qchange_r = np.sum(large_qchanges[:, 1])/len(large_qchanges)
        ...
        # with open(os.path.join(zig_dir, f"ep_{i:02d}_results_lq{large_qchange_l:.02f}_rq{large_qchange_r:.02f}_lerr{l_err:.03f}_rerr{r_err:.03f}_c{np.sum(collisions>0)}.csv"), 'w') as f:
        #     pd.DataFrame(results).to_csv(f, index=False)
        if RENDER:
            writer.release()
        total_errs.append((l_err, r_err, large_qchange_l, large_qchange_r, np.sum(collisions>0)))
        print(f"{zig_idx} Episode {i+1}/{num_ep} done. l_err: {l_err:.03f}, r_err: {r_err:.03f}, large_qchange_l: {large_qchange_l:.02f}, large_qchange_r: {large_qchange_r:.02f}_c{np.sum(collisions>0)}")

    if len(total_errs) == 0: return
    total_errors = np.mean(np.array(total_errs), 0)
    total_error_str = f"l_err_{total_errors[0]:.03f}_r_err_{total_errors[1]:.03f}_lqchange_{total_errors[2]}_rqchange_{total_errors[3]}_collisions_{total_errors[4]}"
