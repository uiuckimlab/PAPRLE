...
viz = MujocoViz(robot)
viz.init_viewer(viewer_width=600, viewer_height=600,FONTSCALE_VALUE=50)
viz.update_viewer(
    azimuth=-175,
    distance=2.0,
    elevation=-20,
...
zarr_path = 'data/flip_result_filtered'
offset_dict = {
...
with zarr.ZipStore(zarr_path) as zip_store:
    ...
    robot0_eef_pos = np.array(data_group['robot0_eef_pos'])
    robot0_eef_pos = kalman_filter_3d(robot0_eef_pos, np.full(len(robot0_eef_pos), 1/30.0))
    ...
    robot1_eef_pos = kalman_filter_3d(robot1_eef_pos, np.full(len(robot1_eef_pos), 1/30.0))
    ...
    zig_dir = os.path.join('data/ziggle_final_new/', f"{zig[0]:.03f}_{zig[1]:.03f}_{zig[2]:.03f}")
    if WRIST_MODE:
    ...
    for i in range(0, num_ep):
        ...
        if RENDER:
            writer = cv2.VideoWriter(os.path.join(zig_dir, f"ep_{i:02d}.mp4"), cv2.VideoWriter_fourcc(*'mp4v'), 30, (672*3, 224*3))
        ...
        for t in range(ep_start, ep_end):
            ...
            if (t-ep_start) < gripper_close[i][0]: pass
            elif (t-ep_start) < gripper_close[i][1]:
                l_gripper_width = [max(l_gripper_width[0], prev_lqpos[-1])]
                r_gripper_width = [max(r_gripper_width[0], prev_rqpos[len(solved_rq)])]
            else:
                l_gripper_width = [min(l_gripper_width[0], prev_lqpos[-1])]
                r_gripper_width = [min(r_gripper_width[0], prev_rqpos[len(solved_rq)])]
            ...
            if RENDER:
                viz.env.plot_T(p=[0, 0, 0], R=np.eye(3), PLOT_AXIS=True, axis_len=0.5, axis_width=0.005, label='T:[%d]' % (t))
            ...
            if prev_lqpos is not None:
                lq_diff = np.linalg.norm(lqpos[:-1] - prev_lqpos[:-1])
                rq_diff = np.linalg.norm(rqpos[:-1] - prev_rqpos[:-1])
                large_qchanges.append((lq_diff > 0.1, rq_diff > 0.1))
            prev_lqpos = lqpos
            prev_rqpos = rqpos

            collisions.append(int(viz.env.data.ncon))

            if RENDER:
                viz.render()
                im = cv2.resize(im, dsize=(448*3, 224*3), interpolation=cv2.INTER_LINEAR)
        ...
        with open(os.path.join(zig_dir, f"ep_{i:02d}_results_lq{large_qchange_l}_rq{large_qchange_r}_lerr{l_err:.03f}_rerr{r_err:.03f}_c{np.sum(collisions>0):05d}.csv"), 'w') as f:
            pd.DataFrame(results).to_csv(f, index=False)
        if RENDER:
            writer.release()
        total_errs.append((l_err, r_err, large_qchange_l, large_qchange_r))
        print(f"Episode {i+1}/{num_ep} done. l_err: {l_err:.03f}, r_err: {r_err:.03f}, large_qchange_l: {large_qchange_l}, large_qchange_r: {large_qchange_r} collision :{np.sum(collisions>0):05d}")

    total_errors = np.mean(np.array(total_errs), 0)
    total_error_str = f"l_err_{total_errors[0]:.03f}_r_err_{total_errors[1]:.03f}_lqchange_{total_errors[2]}_rqchange_{total_errors[3]}_c{np.sum(collisions>0):05d}"
