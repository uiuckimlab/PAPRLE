...
def stream_camera(cam_info, color_img, depth_img, time_stamp, depth_units, shutdown=False):
    ...
    config.enable_stream(rs.stream.depth, width, height, rs.format.z16, cam_info.fps)
    width, height = cam_info.rgb_resolution
    config.enable_stream(rs.stream.color, width, height, rs.format.rgb8, cam_info.fps)
...
class RealSenseReader:
        ...
        for idx, (name, cam_info) in enumerate(camera_config.items()):
            try:
                ...
                self.images[name] = {
                    ...
                    'depth': RawArray('d', np.zeros([cam_info.depth_resolution[1], cam_info.depth_resolution[0]], dtype=np.uint16).flatten()),
    ...
    def render(self):
        ...
        for name in self.images.keys():
            ...
            im = np.ascontiguousarray(np.array(self.images[name]['color']).reshape(h,w, 3).astype(np.uint8)[...,[2,1,0]])
            im = cv2.putText(im, name, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
            if self.render_depth:
                dw, dh = self.cam_infos[name].depth_resolution
