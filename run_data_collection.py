...
class Runner:
    def __init__(self, robot_config, leader_config, env_config):
        ...
        self.leader = LEADERS_DICT[leader_config.type](self.robot, leader_config, env_config, render_mode='human') # Get signals from teleop devices, outputs joint positions or eef poses as teleop commands.
        self.teleop = Teleoperator(self.robot, leader_config, env_config, render_mode='mujoco') # Solving IK for joint positions if not already given, check collision, and output proper joint positions.
        self.env = ENV_DICT[env_config.name](self.robot, leader_config, env_config, render_mode='', leader=self.leader) # Actually send joint positions to the robot.
